    ...
    try {
      ...
      try {
        ...
        if (ordersResponse.ok) {
          ...
          console.warn(
            "Failed to fetch customer orders, showing empty orders list"
          );
          setOrders([]);
        }
      } catch (ordersErr) {
      ...
      setError(
        err.message || "Failed to load customer data. Please try again."
      );
    } finally {
      setLoading(false);
    }
          ...
          <DialogContentText>
            ...
            {dialogAction === "unblock" && (
              <>
                ...
                <strong>{customer.name}</strong>? They will regain full access
                to their account.
              </>
            )}
            {dialogAction === "delete" && (
              <>
                ...
                {customer.orderCount > 0 && (
                  <Typography color="error" sx={{ mt: 1 }}>
                    Warning: This customer has {customer.orderCount} orders.
                    Deleting this customer may affect order history.
                  </Typography>
                )}
              </>
            )}
